...
wire [6:0] funct7;
wire [3:0] alu_sel;
wire [31:0] pc4, pc_branch, next_pc, pc_jal, pc_jalr;
wire jal, jalr;
...
assign next_pc = jal ? pc_jal : jalr ? pc_jalr : branch_taken ? pc_branch : pc4;

assign pc_jal = pc_reg + imm_j;
...
datapath dp1 (.clk(clk), .we(reg_we), .rs1(rs1), .rs2(rs2), .rd(rd), .load_rd(load_rd),
              ...
              .op1(op1), .op2(op2), .alu_out(alu_out), .alu_sel(alu_sel), .branch(branch),
              .branch_taken(branch_taken), .jal(jal), .jalr(jalr), .pc4(pc4));
